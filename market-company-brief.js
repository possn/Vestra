/* Vestra Market Company Brief v2.0 — canonical dossier/company description repair + dossier normalization pipeline. */
(() => {
'use strict';
...
function loadScript(id,src,ready,onload){if(ready){if(onload)onload();return;}const existing=document.getElementById(id);if(existing){if(onload)existing.addEventListener('load',onload,{once:true});return;}const s=document.createElement('script');s.id=id;s.src=src;s.defer=true;if(onload)s.addEventListener('load',onload,{once:true});document.head.appendChild(s)}
...
function loadGlobalMarketSearch(){loadScript('vestra-global-market-search-script','market-global-search.js?v=1.5',window.VestraGlobalMarketSearch)}
...
window.VestraMarketCompanyBrief=Object.freeze({brief,refresh:refreshDossier,version:'2.0'});
})();