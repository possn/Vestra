/* Vestra AI Brief v1.0 — deterministic evidence brief + safe Worker LLM handoff. */
(() => {
'use strict';
const VERSION='1.0';
...
async function runAI(card,s){const base=workerBase();const out=card.querySelector('.ai459-content'),btn=card.querySelector('[data-ai459-run]');if(!base){card.querySelector('.ai459-status').textContent='Worker sem endpoint IA configurado';return}btn.disabled=true;btn.textContent='A analisar…';card.querySelector('.ai459-status').textContent='A cruzar apenas evidência Vestra';try{const r=await fetch(`${base}/ai-brief`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({type:'company_brief',version:'1',data:payload(s)})});if(!r.ok)throw new Error(`HTTP ${r.status}`);const d=normalizeAI(await r.json());if(!d||!d.thesis)throw new Error('Resposta inválida');out.innerHTML=`<div class="ai459-grid"><section><small>TESE</small><p>${esc(d.thesis)}</p></section><section><small>PORQUÊ AGORA</small><p>${esc(d.whyNow||d.catalysts||'—')}</p></section><section><small>RISCOS</small><p>${esc(d.risks||'—')}</p></section><section><small>O QUE MUDA A TESE</small><p>${esc(d.change||'—')}</p></section></div>`;card.dataset.ai459='live';card.querySelector('.ai459-status').textContent='Vestra AI · evidência atual';btn.textContent='Atualizar IA'}catch(e){card.querySelector('.ai459-status').textContent='IA ainda não configurada no Worker · mantém-se o brief local';btn.textContent='Analisar com IA'}finally{btn.disabled=false}}
...
window.VestraAiBrief={version:VERSION,evidence,payload,install};
