...
(() => {
  ...
  function create(options={}){
    ...
    function evidencePanel(s={}){
      ...
      const agreementChecks=number(p.agreement_checks);
      const agreementPct=number(p.agreement_pct);
      const agreementPeriod=text(p.agreement_period_end);
      const agreementVisible=agreementChecks!=null&&agreementChecks>=2&&agreementPct!=null;
      const agreementTone=agreementPct!=null&&agreementPct>=90?'is-positive':agreementPct!=null&&agreementPct<75?'is-risk':'is-warn';
      const agreementNote=agreementVisible?`<p class="market-case-note"><strong class="${agreementTone}">Acordo entre fontes · ${Math.round(agreementPct)}%</strong> · ${Math.round(agreementChecks)} métricas anuais do mesmo exercício${agreementPeriod?` · ${escapeHtml(formatShortDate(agreementPeriod))}`:''}</p>`:'';
      return `<div class="market-detail-card market-evidence-quality"><div class="market-perspective-head"><div><small>QUALIDADE DA EVIDÊNCIA</small><h4>${escapeHtml(sourceText)}</h4></div><span class="market-data-age ${stateTone}">${escapeHtml(stateLabel)}</span></div><div class="market-mini-grid"><div><small>Fontes fundamentais</small><strong>${escapeHtml(sourceLabel)}</strong></div><div><small>Confiança dos dados</small><strong>${escapeHtml(confidenceLabel)}</strong></div><div><small>Cobertura</small><strong>${escapeHtml(coverageLabel)}</strong></div><div><small>Atualidade fundamental</small><strong>${escapeHtml(ageLabel)}</strong></div></div>${agreementNote}${reasons.length?`<p class="market-case-note">${reasons.map(escapeHtml).join(' · ')}</p>`:''}<p class="market-case-note">Analyst, insiders e divulgações políticas são domínios complementares e não contam como segunda confirmação independente dos fundamentais. O acordo entre fontes, quando mostrado, compara apenas métricas anuais do mesmo exercício. Este painel descreve a evidência; não altera o Score Vestra.</p></div>`;
    }
