...
class MarketRuntimeBridgeAndUpdateTests(unittest.TestCase):
    ...
    def test_force_update_is_navigation_only_and_never_wipes_runtime(self):
        ...
        # document capture intercepts the click before its target listener. The
        # manager only clones once to remove listeners that predate safe ownership;
        # later lifecycle retries are deliberately idempotent.
