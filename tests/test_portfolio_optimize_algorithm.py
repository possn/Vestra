...
class PortfolioOptimizeAlgorithmTests(unittest.TestCase):
    ...
    def test_multi_move_plan_never_falls_back_to_worsening_candidate(self):
        ...
        self.assertIn("const sourceSignals=r=>", s)
        self.assertIn("const gateRank=gate==='severe'?3:gate==='high'?2:gate==='watch'?1:0", s)
        self.assertIn("const positionExcess=Math.max(0,positionPct-maxPosition)", s)
        self.assertIn("const sectorExcess=Math.max(0,sectorPct-maxSector)", s)
        self.assertIn("const pressured=gateRank>0||thesisDown||estimatesDown||positionExcess>0||sectorExcess>0||lowConviction", s)
        ...
        self.assertNotIn("const sourcePressure=r=>", s)
        self.assertNotIn("const fallback=planSources.slice()", s)
        self.assertNotIn("const queue=(sources.length?sources:fallback)", s)
