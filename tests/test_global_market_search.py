...
class GlobalMarketSearchTests(unittest.TestCase):
    ...
    def test_remote_search_routes_into_canonical_dossier_owner(self):
        text = GLOBAL.read_text(encoding="utf-8")
        self.assertIn("market?.openLiveStock", text)
        self.assertIn("origin:'global-search'", text)
        self.assertNotIn("DOSSIER GLOBAL · LIVE", text)
        self.assertNotIn("marketSheetContent", text)
    ...
    def test_remote_dossier_ignores_stale_async_responses(self):
        ...
        self.assertGreaterEqual(text.count("if(!ownsRemoteOpen(request,ticker))return false;"), 3)
    ...
    def test_remote_dossier_fetch_has_a_bounded_deadline(self):
        ...
        self.assertIn("await market.openLiveStock(row,{returnView:'market',origin:'global-search'});", text)
        self.assertNotIn("content.innerHTML", text)
