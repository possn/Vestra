...
class NativeMarketLoadingTests(unittest.TestCase):
    ...
    def test_service_worker_matches_native_market_generation(self):
        ...
        self.assertIn('if (request.destination === "style")', sw)
        self.assertIn('event.respondWith(networkFirst(request)); return;', sw)
        self.assertIn('["script", "worker", "manifest"]', sw)
        self.assertNotIn('["script", "style", "worker", "manifest"]', sw)
        self.assertIn('./market-live-overlay.js', sw)
        self.assertIn('./market-static-universe.js', sw)
        self.assertIn('./dashboard-weekly-events.js', sw)
