...
class VestraPortfolioUiStyleTests(unittest.TestCase):
    def test_portfolio_ui_keeps_dynamic_kpi_widths_and_moves_static_styles_to_css(self):
        ...
        self.assertIn("vestra-portfolio-ui.css?v=1.5", js)
        ...
        self.assertIn("ANÁLISES DE APOIO", css)
        self.assertIn("data-vpu-scan-title", css)
        self.assertIn(".market-data-age{display:none!important}", css)
        self.assertIn('.vpu-portfolio[data-vpu-expanded="1"] > .market-portfolio-section', css)
        self.assertIn('.vpu-exit{', css)
        self.assertIn("data-vpu-exit", js)
