/* Vestra Portfolio Dossier Routing v1.4 — canonical row discovery; navigation delegated to VestraNavigation. */
(()=>{'use strict';
const VERSION='1.4';
...
function isControl(target,row){const ctl=target.closest?.('a,button,input,select,textarea,[data-market-watch],[data-collapse-toggle],.market-collapse-toggle,[data-queue-status],[data-vpu-toggle],[data-vpu-tab],[data-vpu-detail]');return !!(ctl&&ctl!==row)}
function openTicker(ticker,sourceNode){const tk=t(ticker).toUpperCase();if(!tk)return false;const nav=window.VestraNavigation;if(!nav?.openCompany)return false;nav.openCompany(tk,{origin:'portfolio',sourceNode});return true}
document.addEventListener('click',e=>{const c=root();if(!c||!c.contains(e.target))return;let row=e.target.closest?.('[data-market-ticker],.market-row,.market-action-row,.market-fresh-row,.market-rebalance-row,.market-research-queue-main');if(!row||!c.contains(row)||isControl(e.target,row))return;let tk=t(row.dataset.marketTicker);if(!tk){tk=tickerFrom(row);if(tk)row.dataset.marketTicker=tk}if(!tk)return;
  ...
  // This fallback covers dynamically generated portfolio suggestions, but navigation
  // itself remains exclusively owned by VestraNavigation.
