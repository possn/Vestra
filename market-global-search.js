...
(() => {
  ...
  function ownsRemoteOpen(request,ticker){
    return request===remoteOpenSeq && txt(ticker).toUpperCase()===ticker;
  }
  ...
  async function openRemoteTicker(ticker){
    ...
    try{
      ...
      if(!ownsRemoteOpen(request,ticker))return false;
      ...
      const row={
        ...d,
        ticker:txt(d.ticker||ticker).toUpperCase(),
        name:txt(d.name||ticker),
        exchange:txt(d.exchange),
        currency:txt(d.currency)||'USD',
        quote_type:txt(d.quote_type||'EQUITY'),
        sector:txt(d.sector),
        industry:txt(d.industry),
        country:txt(d.country),
      };
      await learn(row,'worker-market');
      if(!ownsRemoteOpen(request,ticker))return false;
      const market=window.VestraMarket;
      if(!market?.openLiveStock) throw new Error('Dossier canónico indisponível');
      await market.openLiveStock(row,{returnView:'market',origin:'global-search'});
      return true;
    }catch(e){
      if(request!==remoteOpenSeq)return false;
      console.error('[Vestra global search]',e);
      const market=window.VestraMarket;
      if(market?.openLiveStock){
        await market.openLiveStock({
          ticker,
          name:ticker,
          quote_type:'EQUITY',
          _liveLoadError:txt(e?.message||'Sem dados'),
        },{returnView:'market',origin:'global-search'});
      }
      return false;
    }
  }
