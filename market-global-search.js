/* Vestra Global Market Search v1.9 — canonical global dossiers with exact provider identity. */
(() => {
  'use strict';
  ...
  async function learnCentral(row, timeoutMs=LEARN_FETCH_TIMEOUT_MS){
    ...
    if (!base || !validTickerQuery(ticker) || provider!==ticker || row?.identity_verified!==true || learnedPosted.has(ticker)) return false;
  ...
  async function openRemoteTicker(ticker,options={}){
    ...
    const exact=exactRows.find(row=>txt(row?.ticker).toUpperCase()===ticker && row?.identity_verified===true && txt(row?.provider_symbol).toUpperCase()===ticker);
    if(!exact) return false;

    try{ await market.ensureLoaded?.(); }catch(_){}
    if(request!==remoteOpenSeq) return false;

    const staged=market.upsertRemoteStock({
      ...exact,
      ticker,
      provider_symbol:ticker,
      identity_verified:true,
      current_price:n(exact.price),
      _remoteLoading:true,
      _remoteGlobal:true,
      source:'global-search-exact',
    });
    ...
    try{
      ...
      if(!exactProviderIdentity(ticker,d)) throw new Error('Identidade global não confirmada');
