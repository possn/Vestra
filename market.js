...
function ageText(){ return marketRowUI?.ageText() || ''; }
  ...
  function portfolioIntelligence(rows,total){
    ...
    const scenarioHtml=scenarioRows.length?`<div class="market-detail-card market-scenario-preview"><div class="market-perspective-head"><div><small>SCENARIO PREVIEW</small><h4>Se substituíres pelo mesmo valor</h4></div><span class="market-data-age">simulação</span></div><p class="market-case-note">Mantém o valor da posição e o setor; mostra impacto e elegibilidade separadamente. Evidência limitada não é tratada como deterioração da carteira.</p><div class="market-scenario-list">${scenarioRows.map(x=>`<div class="market-scenario-row"><div><strong>${esc(x.from.ticker)} → ${esc(x.to.ticker)}</strong><small>Convicção carteira ${x.before.toFixed(1)} → ${x.after.toFixed(1)} · overlap ${x.overlapBefore.toFixed(1)}% → ${x.overlapAfter.toFixed(1)}% · risco ${x.riskPenalty.toFixed(1)} · ${esc(x.eligibility)}${x.warnings?.length?' · ⚠ '+esc(x.warnings.join(' · ')):''}</small></div><em class="${x.impact==='Melhora'?'is-positive':x.impact==='Piora'?'is-risk':''}">${x.impact}</em></div>`).join('')}</div><p class="market-case-note">Cenário indicativo: não considera fiscalidade, spreads, comissões ou liquidez. Um cenário não elegível pode continuar visível para comparação manual.</p></div>`:'';
