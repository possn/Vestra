...
function ageText(){ return marketRowUI?.ageText() || ''; }
  ...
  function weeklyEtfConfirmation(label){
    ...
    const flowPct=withFlow.length?rotationMedian(withFlow.map(f=>n(f.fund_flow_1w_pct)).filter(x=>x!=null)):null;
    const etfReturn=withReturn.length?rotationMedian(withReturn.map(f=>n(f.fund_return_1w_pct)).filter(x=>x!=null)):null;
    const days=withFlow.length?Math.max(...withFlow.map(f=>n(f.fund_flow_observation_days)||0)):null;
    return {tickers,funds,withFlow:withFlow.length,withReturn:withReturn.length,flowUsd,flowPct,etfReturn,days};
  }
  function compactFlowUsd(v){
    if(v==null)return 'baseline';
    const sign=v>=0?'+':'-'; const x=Math.abs(v);
    if(x>=1e9)return sign+'
