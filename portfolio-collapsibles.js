...
function setCollapsed(card,x){card.classList.toggle('is-collapsed',x);if(x){card.tabIndex=0;card.setAttribute('role','button');card.setAttribute('aria-label','Abrir secção');}else{card.removeAttribute('tabindex');card.removeAttribute('role');card.removeAttribute('aria-label');}const b=card.querySelector(':scope > .market-collapse-toggle');if(b){b.textContent=x?'＋':'−';b.title=x?'Abrir secção':'Fechar secção';b.setAttribute('aria-expanded',x?'false':'true')}}
...
document.addEventListener('keydown',e=>{if(e.key!=='Enter'&&e.key!==' ')return;const card=e.target.closest?.('.market-detail-card[data-collapsible="1"].is-collapsed');if(!card||e.target!==card)return;e.preventDefault();toggleCard(card)});
