/* Vestra Portfolio Collapsibles v1.7 — canonical toggle owner; Portfolio UI owns direct card intent. */
...
document.addEventListener('click',e=>{const b=e.target.closest?.('[data-collapse-toggle]');if(!b)return;e.preventDefault();e.stopPropagation();toggleCard(b.closest('.market-detail-card'))});
function start(){style()}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start,{once:true});else start();
window.VestraPortfolioCollapsibles=Object.freeze({refresh:install,version:'1.7'});
})();
