...
if NEW in text:
    print('Worker CORS already restricted to Vestra origin')
elif text.count(OLD) != 1:
    raise RuntimeError('Expected current corsHeaders implementation not found exactly once')
else:
    WORKER.write_text(text.replace(OLD, NEW, 1), encoding='utf-8')
    print('Worker CORS restricted to Vestra origin')
