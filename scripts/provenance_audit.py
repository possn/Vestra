...
It measures how much independent evidence supports each dossier, how fresh official
filings are, and where source agreement or freshness is weak by region/model.
"""
...
def summarize(rows: list[dict], today: dt.date | None = None) -> dict:
    ...
    for row in rows:
        ...
        if evidence_state == "observed":
            observed_rows += 1
        independent[_independent_bucket(p.get("independent_source_count"))] += 1
        agreement[_agreement_bucket(p)] += 1
        families = {str(x) for x in (p.get("independent_source_families") or []) if x}
        for family in families:
            source_families[family] += 1
        if families & {"sec_edgar", "esef"}:
    ...
    return {
        ...
        "independent_source_count": dict(independent),
        ...
        "weakest_evidence_dossiers": [
            {
                ...
                "independent_source_count": (row.get("data_provenance") or {}).get("independent_source_count", 0),
                "independent_source_families": (row.get("data_provenance") or {}).get("independent_source_families", []),
