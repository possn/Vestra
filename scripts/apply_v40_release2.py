...
# Enable SEC EDGAR without requiring a workflow-level env change.
one('scripts/sec_enrich.py',"ua=os.getenv('SEC_USER_AGENT','').strip()","ua=os.getenv('SEC_USER_AGENT','Vestra/4.0 (+https://github.com/possn/Vestra)').strip()")
one('scripts/sec_enrich.py','Enabled when SEC_USER_AGENT is set in the environment. Uses official SEC','Enabled by default with an identifiable Vestra User-Agent; SEC_USER_AGENT can override it. Uses official SEC')
one('scripts/sec_enrich.py','User-Agent, so we deliberately do nothing until a proper value is configured.','User-Agent; Vestra ships a repository-identifying default and allows an environment override.')
