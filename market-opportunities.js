/* Vestra Market Opportunities v1.0 — canonical opportunity ranking/rendering. */
(() => {
  'use strict';
  ...
  function opportunities(){
    ...
    const sig=rows.map(s=>`${t(s.ticker)}:${Math.round(score(s)||0)}`).join('|')+sec;if(list.dataset.ux453===sig)return;
  ...
  window.VestraMarketOpportunities=Object.freeze({stats,confirmed,timing,eligible,score,refresh:opportunities});
})();
